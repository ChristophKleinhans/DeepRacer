...
def wrapper(LINE_ITERATIONS, offset):
    ...
    # Filter the waypoints, that the values are not too close, otherwise shapely will make a
    # multilinesting out of it which cant be offsetted

    diff_lines = np.diff(waypoints, axis=0)
    ...
    def offset_to_center(offset):
        center_line_string = sh.LineString(center_line)
        inner_off_string = sh.LineString(center_line_string.offset_curve(offset))
        outer_off_string = sh.LineString(center_line_string.offset_curve(-offset))
        
        inner_border = []
        outer_border = []

        for i in range(0,len(inner_off_string.xy[0]),1):
            inner_border.append([inner_off_string.xy[0][i],
                                inner_off_string.xy[1][i]])
            
        for i in range(0,len(outer_off_string.xy[0]),1):
            outer_border.append([outer_off_string.xy[0][i],
                                outer_off_string.xy[1][i]])
        
        return inner_border, outer_border


    inner_border, outer_border = offset_to_center(offset=offset)


    l_center_line = LineString(center_line)
    l_inner_border = LineString(inner_border)
    l_outer_border = LineString(outer_border)
    flipped = np.flipud(list(l_inner_border.coords))
    line_tuple = (list(l_outer_border.coords), flipped)
    road_poly = Polygon(np.vstack(line_tuple))
    ...
    print(f"Offset: {offset} - Should be same: {(center_line.shape, loop_race_line.shape)} - Original cent length: {l_center_line.length} - new racelin: {LineString(loop_race_line).length}")
    ...
    return loop_race_line

track_width = 0.76
half_track_width = track_width / 2
# for w in np.arange(0.36, half_track_width+0.25, 0.02):
#     wrapper(LINE_ITERATIONS=2000, offset=w)
loop_race_line = wrapper(LINE_ITERATIONS=200, offset=0.5)

now = datetime.now()
prefix = 'reinvent_2018_offset_05'
py_fname = prefix + '.py'
